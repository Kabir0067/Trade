"""
═══════════════════════════════════════════════════════════════════════════
  SIGNAL SCORECARD  —  forward paper-test on the REAL market (Bitcoin 24/7)
═══════════════════════════════════════════════════════════════════════════
...
def _fetch_m1():
    """Latest M1 bars (structured array) — enough to cover the crypto timeout window."""
    try:
        # Мутобиқсозӣ барои Биткоин: Азбаски SCORECARD_TIMEOUT_MIN дар крипто
        # калонтар аст (90-120 дақ), мо барҳои бештар мегирем (max 300).
        bars = max(300, int(config.SCORECARD_TIMEOUT_MIN) + 60)
        # Истифодаи синфи нави Биткоин
        return mc.GetRealBtcData(num_bars=bars).get_data(specific_tf=mt5.TIMEFRAME_M1)
    except Exception:
        return None
...
def format_report():
    ...
    blocks = [_fmt_block("📈 Движок БИТКОИН (24/7)", s[v]) for v in versions]
    ...
    return (
        f"📊 *SCORECARD — форвард-тест на Crypto-рынке*\n"
