...
import config
import mt5_clent as mc        # live broker→UTC offset (for broker-aligned H4 freshness)
...
# ═══════════════════════════════════════════════════════════════════════════
#  INDICATOR ENGINE  —  classic, NaN-safe & self-contained
# ═══════════════════════════════════════════════════════════════════════════
class InstitutionalIndicatorEngine:
    ...
    def compute_all(self):
        df = self.df
        # ── self-contained candle geometry (don't depend on the data loader) ──
        ...
        # wick-rejection (absorption) — independent of body direction
        ...
        df['overextended_up'] = (df['rsi'] > 78) & (df['dist_from_ema200'] > 2.5)
        df['overextended_down'] = (df['rsi'] < 22) & (df['dist_from_ema200'] < -2.5)

        # ── RSI divergence (true: compares vs PRIOR window, excludes current bar) ──
        ...
        df['high_volume'] = volume > df['vol_sma'] * 1.5
        ...
        # ── Rolling VWAP + σ-bands (institutional fair value) ──
        w = 20
        ...
        # ── Volume Profile POC + value area (rolling approximation) ──
        df['poc_level'] = vp / vv
        ...
        # ── Volatility regime + squeeze (percentile ranks) ──
        ...
        # BOS / liquidity / FVG (all guarded against NaN comparisons)
...
# ═══════════════════════════════════════════════════════════════════════════
#  SESSION & CANDLE TIMING  (injectable `now` for tests)
# ═══════════════════════════════════════════════════════════════════════════
# DST-aware session zones (handles winter/summer automatically, any broker tz).
try:
    ...
    _ZONES_OK = True
except Exception:                       # tzdata missing -> fall back to UTC windows
    _ZONES_OK = False


def _in_window(dt, sh, sm, eh, em):
    """True if dt's local clock is within [sh:sm, eh:em) AND it's a weekday there."""
    if dt.weekday() >= 5:               # Sat/Sun — market session not active
        return False
    minutes = dt.hour * 60 + dt.minute
    return sh * 60 + sm <= minutes < eh * 60 + em


def get_session_info(now=None, broker_offset_h=0):
    """
    Detect the active FX/gold session and candle freshness.
    Session is derived from the real local market clock (London/NY/Tokyo) via
    zoneinfo, so it stays correct across DST (winter/summer) and is independent
    of both the broker's server timezone AND the host server's local timezone —
    it works automatically on ANY server. `now` is a tz-aware UTC datetime.

    `broker_offset_h` (live broker→UTC hour offset) only aligns the H4-freshness
    bonus to the broker's candle clock; it does NOT affect session detection.
    """
    if now is None:
        now = datetime.now(timezone.utc)
    if now.tzinfo is None:
        now = now.replace(tzinfo=timezone.utc)
    hour, minute = now.hour, now.minute   # UTC — used for candle-open timing

    if _ZONES_OK:
        ldn = now.astimezone(_TZ_LONDON)
        ny = now.astimezone(_TZ_NY)
        tok = now.astimezone(_TZ_TOKYO)
        london = _in_window(ldn, 8, 0, 16, 30)    # London FX/gold hours
        newyork = _in_window(ny, 8, 0, 17, 0)     # New York FX/gold hours
        asian = _in_window(tok, 9, 0, 15, 0)      # Tokyo hours
        if london and newyork:
            session, power = "LONDON_NY_OVERLAP", 1.15   # peak gold volatility
        elif london:
            session, power = "LONDON", 1.10
        elif newyork:
            session, power = "NEW_YORK", 1.08
        elif asian:
            session, power = "ASIAN", 0.90
        else:
            session, power = "OFF_HOURS", 0.85
    else:
        if 13 <= hour < 15:
            session, power = "LONDON_NY_OVERLAP", 1.15
        elif 7 <= hour < 13:
            session, power = "LONDON", 1.10
        elif 15 <= hour < 21:
            session, power = "NEW_YORK", 1.08
    ...
    # H4 candles are aligned to the BROKER's server clock (e.g. Exness EET = UTC+2/+3),
    # NOT to UTC — so shift to the broker hour before testing "an H4 just opened". The
    # offset is derived live from the feed, so this is correct & automatic on ANY broker
    # timezone. (H1 opens at :00 and M15 at :00/:15/:30/:45 — offset-invariant for
    # whole-hour brokers — so their UTC-minute tests already line up.)
    broker_hour = (hour + int(broker_offset_h)) % 24
    h4_fresh = ((broker_hour % 4) * 60 + minute) <= 10
    h1_fresh = minute <= 5
    m15_since = minute % 15
...
# ═══════════════════════════════════════════════════════════════════════════
#  PER-TIMEFRAME SCORING  —  NaN-safe, regime-adaptive (Kaufman-efficiency + ADX).
#  Five confluence blocks: SMC structure / momentum / mean-reversion / candle &
#  divergence confirmation / volume-flow — each capped, then regime-weighted.
# ═══════════════════════════════════════════════════════════════════════════
def _score_timeframe(c, p):
    """Score one timeframe's last closed candle. Returns (buy, sell, breakdown)."""
    # ── RAW (unweighted) sub-scores — shared by both engine versions ──
    # 1. SMC structure (cap 20)
    smc_b = smc_s = 0
    ...
    if mh > 0 and mh > pmh:          # positive & rising
        mom_b += 6
    if mh < 0 and mh < pmh:
        mom_s += 6
    ...
    # 4. Confirmation: candles + divergence + EMA stack (cap 15)
    conf_b = conf_s = 0
    if (num(c.get('engulfing')) > 0 or num(c.get('hammer')) > 0
            or c.get('pin_bar_bull') or c.get('rej_bull')):
        conf_b += 5
    if c.get('bullish_divergence') or c.get('cvd_bull_div'):
        conf_b += 5
    if c.get('ema_bull_stack'):
        conf_b += 5
    if (num(c.get('engulfing')) < 0 or num(c.get('shooting_star')) > 0
            or c.get('pin_bar_bear') or c.get('rej_bear')):
        conf_s += 5
    ...
    # ── REGIME WEIGHTS (Kaufman efficiency + ADX): trust trend-blocks in a trend,
    #    reversion-block in a range ──
    regime = c.get('regime', 'mixed')
    trend_w = 1.25 if regime == 'trend' else (0.70 if regime == 'range' else 1.0)
    rev_w = 1.25 if regime == 'range' else (0.65 if regime == 'trend' else 1.0)
    ...
    # ── Overextension penalty (RSI + distance-from-EMA200 based) ──
...
class MultiTimeframeSignalEngine:
    def __init__(self, data_engine):
        ...
        self._last_dir_ts = {"BUY": -1e18, "SELL": -1e18}   # anti-over-trade clock
        self._pending = None  # (key, dir, ts) awaiting commit_pending() on deliver

    # ── scalp-sized SL/TP from real structure ──────────────────────────────
    def _compute_smart_sl_tp(self, signal_dir, exec_df, m15_df, price, atr):
        ...
        try:
            ...
            spread = 0.0
        spread_buffer = max(spread * 0.5, atr * 0.03)   # at least 3% of ATR
        ...
        for df in [exec_df, m15_df]:
            ...
            # liquidity (swing pivots) — placed like a disciplined trader: the SL
            # sits a touch BEYOND the level (a stop-hunt wick won't take it), the TP
            # a touch BEFORE the level (it fills before the crowd reverses price right
            # at the obvious high/low). Raw-at-the-level placement is what makes a
            # multi-TF-correct signal still stop out / just miss TP.
            sl_buf, tp_buf = atr * 0.10, atr * 0.08
        ...
        def best_tp(risk):
            # prefer the nearest structural target that still gives >= MIN_RR
            band = config.TP_ATR_MULT_MAX * atr
            if signal_dir == 'BUY':
                v = [lvl for lvl, w in tp_c
                     if price + risk * config.MIN_RR <= lvl <= price + band]
                if v:
                    return min(v)
                return price + max(risk * config.TARGET_RR, atr)
            else:
                v = [lvl for lvl, w in tp_c
                     if price - band <= lvl <= price - risk * config.MIN_RR]
                if v:
                    return max(v)
                return price - max(risk * config.TARGET_RR, atr)

        sl = best_sl()
        # Apply spread buffer: push SL further from price by the buffer amount
        if signal_dir == 'BUY':
            sl -= spread_buffer
        else:
            sl += spread_buffer
        ...
        if risk > max_sl or risk <= 0:                       # cap to scalp stop
            sl = price - max_sl if signal_dir == 'BUY' else price + max_sl
            risk = max_sl
        elif risk < min_sl:                                  # floor so noise/spread can't trip it
            sl = price - min_sl if signal_dir == 'BUY' else price + min_sl
            risk = min_sl
        tp = best_tp(risk)
        reward = abs(tp - price)
        if reward > config.TP_ATR_MULT_MAX * atr:            # cap target distance
            tp = (price + config.TP_ATR_MULT_MAX * atr if signal_dir == 'BUY'
                  else price - config.TP_ATR_MULT_MAX * atr)
            reward = config.TP_ATR_MULT_MAX * atr
        if reward < risk * config.MIN_RR:                    # enforce RR floor
            tp = (price + risk * config.TARGET_RR if signal_dir == 'BUY'
                  else price - risk * config.TARGET_RR)
    ...
    # ── aggregate per-TF scores into a weighted multi-TF view ───────────────
    ...
    def _decide(self, aggr, shared):
        ...
        # candle-timing bonus only to the LEADING side (no double-count)
        ...
        sess_map = {"LONDON_NY_OVERLAP": "🔥 LDN+NY", "LONDON": "🇬🇧 London",
                    "NEW_YORK": "🇺🇸 New York", "ASIAN": "🌙 Asian", "OFF_HOURS": "💤 Off"}
        ...
        # DECISION: anchor (M15/M5) confirms + >=MIN_TF_AGREE agree + conf gate
        ...
        # anti-chasing: don't enter when price is already stretched > OVEREXT_Z σ
        # from its 20-bar mean on the exec TF (classic z-score, no Kalman needed)
        ext = config.OVEREXT_Z
        exec_z = shared.get('exec_z', 0.0)
        if signal_dir == "BUY" and exec_z >= ext:
            return {**base, "signal": "NEUTRAL",
                    "reason": f"Overextended +{exec_z:.1f}σ — not chasing the top"}
    ...
    @staticmethod
    def _ab_view(d):
        """Minimal per-version view the scorecard needs (un-deduped)."""
        return {
    ...
    def generate_signal(self, now=None):
        ...
        # ── single pass: compute features per TF once ──
        per_tf = []
        ...
        try:
            broker_off = mc.get_server_utc_offset_hours()
        except Exception:
            broker_off = 0
        # Session detection anchored to the BROKER clock (robust on any server, even
        # with a skewed OS clock) — same approach as the BTC bot. An explicit `now`
        # (tests) still overrides; broker_offset_h keeps H4-freshness broker-aligned.
        session_now = now
        if session_now is None and hasattr(self.data_engine, 'broker_utc_now'):
            try:
                ...
                session_now = None
        timing = get_session_info(now=session_now, broker_offset_h=broker_off)
        ...
        if s.get("signal") in ("BUY", "SELL"):
            ...
            cooled = 0 <= elapsed < config.SAME_DIR_COOLDOWN_MIN * 60   # clock-back => not cooled
            suppress = None
            ...
            if suppress:
                ...
                self._pending = (key, d, now_ts)   # commit only once actually delivered

        # scorecard records this one live engine's signals (un-deduped view)
        s["engine_version"] = "live"
        s["ab"] = {"live": self._ab_view(decision)}
        return s

    def commit_pending(self):
        """Advance the dedupe + same-direction cooldown clocks. Call ONLY after the
        last generate_signal()'s alert was actually delivered, so a dropped alert
        never starts the cooldown window (fixes intent-vs-delivery desync)."""
        if self._pending:
