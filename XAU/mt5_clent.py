...
# Cached broker→UTC offset in hours (e.g. Exness EET would be +2/+3; this trial = 0).
# Re-derived on a TTL so a one-off stale read self-heals and a broker DST switch
# (+2↔+3) is picked up within the window instead of sticking for the whole run.
...
def connect_to_mt5():
    try:
        ...
        if info is not None:
            # Already logged in. Re-login ONLY if a real target account is set and
            # it differs — never login(0), which would drop a healthy session.
        ...
        # Not logged in — need credentials WITH a real login.
...
def resolve_symbol():
    """Pick the broker's gold symbol; cache into module + config SYMBOL."""
    global SYMBOL
    SYMBOL = "XAUUSDm"
...
def get_server_utc_offset_hours():
    """Broker-server clock offset from real UTC, in whole hours. UNIVERSAL: works
    on any VPS/country because it is derived from the live tick time vs real UTC,
    not from the local machine timezone. Only a SANE FX offset (UTC-12..+14) is
    cached; an out-of-range reading means a stale tick (market closed) or an
    unsynced local clock, so we return 0 (treat server epoch as UTC) WITHOUT
    caching and retry on the next call once a fresh tick is available.
    NOTE: session detection (London/NY/Tokyo) does NOT use this — it uses real
    UTC via zoneinfo, so sessions are already correct on every server/timezone."""
    ...
    if _SERVER_UTC_OFFSET_H is not None and (now - _SERVER_UTC_OFFSET_TS) < _OFFSET_TTL_SEC:
        return _SERVER_UTC_OFFSET_H          # cached value still fresh (within TTL)
    try:
        tick = mt5.symbol_info_tick(SYMBOL)
        if tick and tick.time:
            off = int(round((tick.time - now) / 3600.0))
            if -12 <= off <= 14:                 # plausible broker offset -> trust + cache
                _SERVER_UTC_OFFSET_H = off
                _SERVER_UTC_OFFSET_TS = now
                return off
            # else: garbage (stale tick / unsynced clock) -> fall through
    except Exception:
        pass
    # no fresh sane reading: keep last good value if any (better than whipsawing to 0)
...
def value_per_price_unit():
    ...
    Reads the broker's real tick value/size so risk sizing matches what the
    account actually uses — instead of assuming CONTRACT_SIZE. Falls back to
    CONTRACT_SIZE only if the broker doesn't expose tick metadata."""
    try:
...
def is_market_open():
    try:
        ...
        tick = mt5.symbol_info_tick(SYMBOL)
        if tick and tick.time:
            # tick.time is broker-server epoch (= real_utc + offset); convert it
            # back to real UTC before measuring age. (Same sign convention as
            # last_bar_age_seconds — the old `age -= offset` doubled the error and
            # mis-flagged a live EET broker as closed.)
            offset = get_server_utc_offset_hours() * 3600
            age = datetime.now(timezone.utc).timestamp() - (tick.time - offset)
            if abs(age) < 120:
                return True
            # stale tick -> likely closed/frozen
            return _weekend_guess()
        return _weekend_guess()
    except ConnectionError:
        raise
    except Exception as e:
        # Fail CLOSED: a trading gate must not assume "open" on an unknown error.
...
def _weekend_guess():
    ...
    d, h = now.weekday(), now.hour     # Mon=0 .. Sun=6
    if d == 5:                         # Saturday — always closed
        return False
    if d == 4 and h >= 20:             # Friday late (1h early buffer)
        return False
    if d == 6 and h < 23:              # Sunday before reopen (1h late buffer)
        return False
    return True


def market_should_be_open():
    """True if, by the calendar/clock, the gold market SHOULD be trading now. Used
    to tell a genuinely-closed market (weekend) apart from a HUNG terminal that has
    simply stopped delivering data during open hours."""
    return _weekend_guess()
...
def _dynamic_deviation_points(symbol_info, spread=0.0, risk=None):
    """Convert a USD/ATR slippage BUDGET into broker deviation points, instead of a
    blind hardcoded deviation. Budget = at most MAX_SLIPPAGE_USD, and at least a
    fraction of the trade's own risk / the live half-spread, then clamped to the
    [MIN, MAX]_DEVIATION_POINTS band. Caps the real slippage on every entry."""
...
def _order_check_error(request):
    """Pre-flight an order with mt5.order_check (margin / stops / volume) BEFORE it
    goes live. Returns an error string to abort on, or None if the order is valid.
    Pure safety: it only blocks an order the broker would reject anyway — it does
    NOT change which signals fire. Disable with ENABLE_ORDER_CHECK=0 in .env."""
...
def _normalize_stops(symbol_info, order_type, price, sl, tp):
    """Round SL/TP to symbol digits and enforce the broker's minimum stop distance."""
...
def open_order(order_type, lot_size, sl=None, tp=None, risk=None, reward=None,
               signal_price=None, comment="GoldSignalBot"):
    ...
    # reject blown-out spreads (news / market open) before committing capital
    ...
    # fine and allowed through. SL/TP re-anchor to the live fill, so this blocks
    # gross chasing, not normal manual-tap latency.
    ...
    # anchor SL/TP to the LIVE price using the requested distances
    ...
    # instead of a blind hardcoded deviation (was 30) — caps real slippage.
...
def open_order_xauusd(order_type, lot_size, sl, tp):
    """Backwards-compatible wrapper (price arg ignored — a fresh price is used)."""
...
def close_position(ticket):
    """Close an open position by ticket (used by the live test). Returns dict."""
    ...
    request = {
        ...
        "magic": config.MAGIC, "comment": "GoldSignalBot close",
...
def set_tp_in_usd(target_profit_usd: float):
    ...
    point = si.point
    # MT5 SymbolInfo uses trade_tick_size / trade_tick_value (NOT tick_size).
...
#  DATA ENGINE
# ───────────────────────────────────────────────────────────────────────────
class GetRealXauData:
    """Fetches gold OHLCV data from MT5 for any timeframe and builds a clean frame."""
    ...
    def last_bar_age_seconds(self, tf):
        """Seconds since the last (forming) bar opened, in real UTC. None on failure."""
    ...
    def broker_utc_now(self):
        ...
        clock. Session detection uses this so London/NY/Asian sessions stay correct
        on ANY server, regardless of its local timezone OR a drifting/skewed OS clock:
        broker_time − broker_utc_offset cancels out a skewed local clock (the offset
        rounds to the nearest hour, so any skew under ~30 min is absorbed). Falls back
        to the OS UTC clock only if no tick is available yet."""
        try:
            tick = mt5.symbol_info_tick(SYMBOL)
            if tick and tick.time:
