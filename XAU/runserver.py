"""
═══════════════════════════════════════════════════════════════════════════
  Gold Signal Bot — Windows production watchdog (DevOps, 24/7)
═══════════════════════════════════════════════════════════════════════════
...
On --install the server clock is first forced onto FRANKFURT time
(W. Europe Standard Time = Europe/Berlin). This makes every market-session
calculation in the bot line up with the FX / metals trading day.
...
Other useful commands:
    python runserver.py --status          # show health + scheduled tasks
    python runserver.py --set-timezone     # force Frankfurt time (needs Admin)
    python runserver.py --kill --close-all # stop everything AND flatten positions
...
BOOT_TASK_NAME      = "GoldSignalBotBoot"
LOGON_TASK_NAME     = "GoldSignalBotLogon"
FALLBACK_LOCK_PORT  = 49346     # singleton guard (distinct from other bots)
...
# ─────────────────────────────────────────────────────────────────────────────
# Frankfurt timezone (align the server clock to the FX / metals trading day)
# ─────────────────────────────────────────────────────────────────────────────
# Windows ID for Europe/Berlin — Frankfurt, Amsterdam, Zurich (CET / CEST, DST-aware).
FRANKFURT_TZ_ID = "W. Europe Standard Time"


def get_windows_timezone() -> Optional[str]:
    """Current Windows timezone ID (e.g. 'W. Europe Standard Time'), or None."""
    if not is_windows():
        return None
    try:
        ...
        log.debug("tzutil /g failed: %s", exc)
        return None


def _resync_clock() -> None:
    """Best-effort NTP resync so the wall clock is accurate after a tz change.
    Needs Administrator; every step is non-fatal (e.g. service already running)."""
    if not is_windows() or not is_admin():
        return
    for cmd in (["sc", "config", "w32time", "start=", "auto"],
                ["net", "start", "w32time"],
                ["w32tm", "/resync", "/force"]):
        try:
            run_command(cmd)          # logged, never raises
        except Exception as exc:
            log.debug("clock step %s failed (non-fatal): %s", cmd[0], exc)


def ensure_frankfurt_timezone() -> bool:
    """Force the Windows server clock onto Frankfurt time (W. Europe Standard Time).

    Idempotent: if it is already Frankfurt, it only resyncs the clock. Changing the
    system timezone requires Administrator — without it, we warn and continue (the
    bot still runs; sessions just rely on whatever tz the host already has).
    Returns True if the timezone is Frankfurt after this call.
    """
    if not is_windows():
        log.info("Timezone: non-Windows host — skipping (set the OS TZ to Europe/Berlin manually).")
        return False

    current = get_windows_timezone()
    if current == FRANKFURT_TZ_ID:
        log.info("Timezone already Frankfurt (%s) — resyncing clock only.", current)
        _resync_clock()
        return True

    if not is_admin():
        log.warning("Server timezone is '%s', NOT Frankfurt. Run ONCE 'As administrator' "
                    "so it can be set to '%s'. (continuing without changing it)",
                    current or "unknown", FRANKFURT_TZ_ID)
        return False

    ok = run_command(["tzutil", "/s", FRANKFURT_TZ_ID]).returncode == 0
    if ok:
        log.info("✅ Timezone set to FRANKFURT: %s (was %s)", FRANKFURT_TZ_ID, current or "unknown")
        _resync_clock()
        new = get_windows_timezone()
        log.info("Verified timezone: %s", new)
    else:
        log.error("Failed to set timezone to %s", FRANKFURT_TZ_ID)
    return ok
...
def write_launcher_bat() -> Path:
    ...
    content = (
        "@echo off\n"
        "setlocal\n"
        "title Gold Signal Bot RunServer\n"
        'cd /d "%~dp0"\n'
...
def install(start_now: bool = False) -> int:
    ...
    ensure_project_ready()
    # STEP 1 — align the server clock to Frankfurt BEFORE anything else, so the
    # bot's market-session math is correct from the very first run.
    ensure_frankfurt_timezone()
...
def run_watchdog(*, no_network_wait: bool = False, no_preflight: bool = False,
    ...
    try:
        ...
        threading.Thread(target=health_tick_loop, args=(stop_event, health, 60.0, health_lock),
                         daemon=True).start()
        telegram_alert(env, f"🟢 Gold Bot runserver started\nServer: {socket.gethostname()}\n"
                            f"Time: {_now_utc()}")
    ...
    telegram_alert(env, f"🔴 Gold Bot runserver stopped\nServer: {socket.gethostname()}\n"
                        f"Restarts: {total}\nTime: {_now_utc()}")
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Gold Signal Bot RunServer — Windows production watchdog.",
    ...
    p.add_argument("--run", action="store_true",
                   help="Run the watchdog in the foreground (used by the launcher .bat / tasks).")
    ...
    p.add_argument("--close-all", action="store_true",
                   help="With --kill, also flatten every open position of this bot.")
    p.add_argument("--status", action="store_true", help="Print health + scheduled-task status.")
    p.add_argument("--set-timezone", action="store_true",
                   help="Force the server clock onto Frankfurt time (needs Administrator).")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        ...
        if args.set_timezone:
            return 0 if ensure_frankfurt_timezone() else 1
        if args.kill:
            return kill_switch(close_all=args.close_all)
        if args.install:
            return install(start_now=args.start_now)
        # --run, or no args at all (Task Scheduler / launcher .bat): run the watchdog.
        return run_watchdog()
    except RunServerError as exc:
