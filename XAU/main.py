"""
═══════════════════════════════════════════════════════════════════════════
  GOLD SIGNAL BOT  —  Telegram front-end + 24/7 analysis loop
═══════════════════════════════════════════════════════════════════════════
"""
...
# ───────────────────────────────────────────────────────────────────────────
#  MONEY MANAGEMENT  ($250 -> 0.01 lot, hard cap MAX_LOT)
# ───────────────────────────────────────────────────────────────────────────
...
def get_risk_based_lot(sl_distance):
    ...
    risk is exact on ANY account type, not an assumed CONTRACT_SIZE."""
    bal = mc.get_balance()
...
def _button_lots(base, score=0):
    """Dynamic lot choices based on signal confidence, clamped to [MIN_LOT, MAX_LOT]."""
    if score >= 90:
        num_buttons = 5
    elif score >= 80:
        num_buttons = 3
    ...
    return sorted({clamp(round(base * m, 2), config.MIN_LOT, config.MAX_LOT)
                   for m in range(1, num_buttons + 1)})
...
def bot_commands():
    return [
        ...
        BotCommand("stats", "📊 Статистика сигналов (forward-тест)"),
    ]
...
def background_analysis_loop():
    ...
    print("🤖 Gold Signal Bot — 24/7 institutional analysis (10-30 min horizon)...")

    # Build the engine ONCE: its zone-dedupe + same-direction cooldown live in
    # instance state, so a fresh instance each cycle would reset them and re-fire
    # duplicates every SIGNAL_COOLDOWN. The data feed re-fetches live bars on every
    # get_data() call, so reusing one instance is correct.
    engine = MultiTimeframeSignalEngine(
        mc.GetRealXauData(timeframe=mt5.TIMEFRAME_M5, num_bars=config.NUM_BARS))
        ...
        try:
            ...
            try:
                if not mc.is_market_open():
                    # Distinguish a real close (weekend) from a HUNG terminal that
                    # should be open but stopped delivering data — auto kill+relaunch
                    # the terminal in the latter case (needs MT5_PATH).
                    if mc.market_should_be_open() and config.MT5_PATH:
                        ...
                        if _stale_since == 0.0:
                            ...
                            print("🔧 Feed stale during open hours — auto-restarting MT5 terminal...")
                            print("✅ MT5 terminal restarted" if mc.restart_terminal()
                                  else "❌ MT5 terminal restart failed (check MT5_PATH)")
                            _stale_since = 0.0
                    print("🛑 Gold market closed / stale feed. Waiting...")
            ...
            # forward paper-test scorecard (records BOTH v1/v2, grades vs real M1
            # path) — observation only, never trades; safe-guarded internally
...
def send_welcome(message):
    bot.reply_to(
        message,
        "🏆 *Gold Signal Bot — Institutional Grade*\n\n"
        "Автоанализ XAUUSDm 24/7 для сделок 10-30 мин\n"
        "• 6 таймфреймов (M1 → D1), M15/M5 — якорь\n"
        "• SMC + Order Flow (CVD) + VWAP + Volume Profile\n"
        "• Режим (Kaufman+ADX) + точные SL/TP по структуре\n"
...
def send_stats(message):
    """/stats — forward paper-test scorecard (v1 classic vs v2 math A/B)."""
    try:
    ...
    try:
        ...
        bot.reply_to(message, report)          # fallback if Markdown parse fails
...
def send_tp_menu(message):
    ...
    row = []
    for i in range(1, 11):
        row.append(InlineKeyboardButton(f"${i}", callback_data=f"SET_TP_{i}"))
        if len(row) == 5:
            markup.row(*row)
            row = []
    if row:
...
def send_sl_menu(message):
    ...
    row = []
    for i in range(1, 11):
        row.append(InlineKeyboardButton(f"${i}", callback_data=f"SET_SL_{i}"))
        if len(row) == 5:
            markup.row(*row)
            row = []
    if row:
...
def _clear_buttons(call):
    """Remove the inline keyboard from the message after a button was pressed,
    so it can't be tapped again (clean UX). Best-effort."""
    try:
...
def handle_trade_execution(call):
    ...
    try:
        ...
        # don't execute a stale tap — the setup has moved on (anti-chasing)
        if signal_ts and time.time() - signal_ts > config.MAX_SIGNAL_EXECUTION_AGE_SEC:
            _clear_buttons(call)
            bot.answer_callback_query(call.id, "⏱ Сигнал устарел — не открываю", show_alert=True)
            return
...
def handle_tp_sl_setting(call):
    try:
        ...
        _clear_buttons(call)               # remove the $1–$10 buttons after choice
...
def send_time_check(message):
    ...
    try:
        ...
        broker_utc = mc.GetRealXauData().broker_utc_now()
        timing = get_session_info(now=broker_utc, broker_offset_h=offset)
...
# ───────────────────────────────────────────────────────────────────────────
def _build_bot():
    """Create the TeleBot and register handlers (no secret defaults in source)."""
    global bot
...
def main():
    ...
    print("🏆 Gold Signal Bot LIVE — Institutional Analysis Active")
    while True:
        try:
